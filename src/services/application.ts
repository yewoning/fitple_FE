...
export function generateApplicationIntro(
  payload: ApplicationAiGenerateRequest,
) {
  return withDemoFallback(
    () =>
      requestRaw<ApplicationAiGenerateResponse>(
...
export function submitApplication(
  ...
  return withDemoFallback(
    ...
    () =>
      demoStore
        .getState()
        .submitApplication(Number(projectId), memberId, payload),
  );
}
...
export function getMyApplications(memberId: number) {
  return withDemoFallback(
    () =>
      requestRaw<MyApplicationItem[]>(
        `/api/applications/my?memberId=${memberId}`,
      ),
