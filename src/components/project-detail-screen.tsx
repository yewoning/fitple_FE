...
import { createMockProjectDetail } from '@/components/project-detail-screen.mock';
...
export function ProjectDetailScreen({ projectId }: ProjectDetailScreenProps) {
  ...
  const loadProject = useCallback(async () => {
    ...
    try {
      ...
      // API 연동 실패 시 목업 데이터로 자연스럽게 대체
      setProject(createMockProjectDetail(projectId));
    } finally {
      setIsLoading(false);
    }
  }, [projectId]);
  ...
  async function handleBookmarkPress() {
    ...
    try {
      ...
      // API 연동 실패 시에도 성공한 것처럼 자연스럽게 채움(다른 화면의 목업 폴백 원칙과 동일)
    } finally {
